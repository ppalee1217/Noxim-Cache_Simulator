...
void NoC::buildMesh()
{
	...
	datareq = new sc_signal_NSWEH<bool> *[dimX];
	dataack = new sc_signal_NSWEH<bool> *[dimX];
	databuffer_full_status = new sc_signal_NSWEH<TBufferFullStatus> *[dimX];
	dataflit = new sc_signal_NSWEH<DataFlit> *[dimX];

	datafree_slots = new sc_signal_NSWE<int> *[dimX];
	datanop_data = new sc_signal_NSWE<NoP_data> *[dimX];
	...
	for (int i = 0; i < dimX; i++)
	{
		...
		datareq[i] = new sc_signal_NSWEH<bool>[dimY];
		dataack[i] = new sc_signal_NSWEH<bool>[dimY];
		databuffer_full_status[i] = new sc_signal_NSWEH<TBufferFullStatus>[dimY];
		dataflit[i] = new sc_signal_NSWEH<DataFlit>[dimY];

		datafree_slots[i] = new sc_signal_NSWE<int>[dimY];
		datanop_data[i] = new sc_signal_NSWE<NoP_data>[dimY];
	}

	t = new Tile **[GlobalParams::mesh_dim_x-1];
	for (int i = 0; i < GlobalParams::mesh_dim_x; i++)
		t[i] = new Tile *[GlobalParams::mesh_dim_y];
    //! Modified
    t_cache = new TileCache **[1];
    t_cache[0] = new TileCache *[GlobalParams::mesh_dim_y];
    //!
	// Create the mesh as a matrix of tiles
	for (int j = 0; j < GlobalParams::mesh_dim_y; j++)
	{
		for (int i = 0; i < GlobalParams::mesh_dim_x; i++)
		{
            ...
            if((i+1)%5 == 0){
                ...
                sprintf(tile_name, "Tile_Cache[%02d][%02d]_(#%d)", i, j, tile_id);
                t_cache[0][j] = new TileCache(tile_name, tile_id);
                // Tell to the router its coordinates
                t_cache[0][j]->r->configure(tile_id,
                GlobalParams::stats_warm_up_time,
                GlobalParams::buffer_depth,
                grtable);
                t_cache[0][j]->r->power.configureRouter(GlobalParams::flit_size,
                GlobalParams::buffer_depth,
                GlobalParams::flit_size,
                string(GlobalParams::routing_algorithm),
                "default");
                t_cache[0][j]->dr->configure(tile_id,
                GlobalParams::stats_warm_up_time,
                GlobalParams::buffer_depth,
                grtable);
                t_cache[0][j]->dr->power.configureRouter(GlobalParams::flit_size,
                GlobalParams::buffer_depth,
                GlobalParams::flit_size,
                string(GlobalParams::routing_algorithm),
                "default");
                // Tell to the PE its coordinates
                t_cache[0][j]->c_nic->local_id = tile_id;
                // Check for traffic table availability
                if (GlobalParams::traffic_distribution == TRAFFIC_TABLE_BASED)
                {
                    ...
                    t_cache[0][j]->c_nic->never_transmit = false;
                // Map clock and reset
                t_cache[0][j]->clock(clock);
                t_cache[0][j]->reset(reset);
                // Map Rx signals
                t_cache[0][j]->req_rx[DIRECTION_NORTH](req[i][j].south);
                t_cache[0][j]->flit_rx[DIRECTION_NORTH](flit[i][j].south);
                t_cache[0][j]->ack_rx[DIRECTION_NORTH](ack[i][j].north);
                t_cache[0][j]->buffer_full_status_rx[DIRECTION_NORTH](buffer_full_status[i][j].north);

                t_cache[0][j]->req_rx[DIRECTION_EAST](req[i + 1][j].west);
                t_cache[0][j]->flit_rx[DIRECTION_EAST](flit[i + 1][j].west);
                t_cache[0][j]->ack_rx[DIRECTION_EAST](ack[i + 1][j].east);
                t_cache[0][j]->buffer_full_status_rx[DIRECTION_EAST](buffer_full_status[i + 1][j].east);

                t_cache[0][j]->req_rx[DIRECTION_SOUTH](req[i][j + 1].north);
                t_cache[0][j]->flit_rx[DIRECTION_SOUTH](flit[i][j + 1].north);
                t_cache[0][j]->ack_rx[DIRECTION_SOUTH](ack[i][j + 1].south);
                t_cache[0][j]->buffer_full_status_rx[DIRECTION_SOUTH](buffer_full_status[i][j + 1].south);

                t_cache[0][j]->req_rx[DIRECTION_WEST](req[i][j].east);
                t_cache[0][j]->flit_rx[DIRECTION_WEST](flit[i][j].east);
                t_cache[0][j]->ack_rx[DIRECTION_WEST](ack[i][j].west);
                t_cache[0][j]->buffer_full_status_rx[DIRECTION_WEST](buffer_full_status[i][j].west);

                // Map Tx signals
                t_cache[0][j]->req_tx[DIRECTION_NORTH](req[i][j].north);
                t_cache[0][j]->flit_tx[DIRECTION_NORTH](flit[i][j].north);
                t_cache[0][j]->ack_tx[DIRECTION_NORTH](ack[i][j].south);
                t_cache[0][j]->buffer_full_status_tx[DIRECTION_NORTH](buffer_full_status[i][j].south);

                t_cache[0][j]->req_tx[DIRECTION_EAST](req[i + 1][j].east);
                t_cache[0][j]->flit_tx[DIRECTION_EAST](flit[i + 1][j].east);
                t_cache[0][j]->ack_tx[DIRECTION_EAST](ack[i + 1][j].west);
                t_cache[0][j]->buffer_full_status_tx[DIRECTION_EAST](buffer_full_status[i + 1][j].west);

                t_cache[0][j]->req_tx[DIRECTION_SOUTH](req[i][j + 1].south);
                t_cache[0][j]->flit_tx[DIRECTION_SOUTH](flit[i][j + 1].south);
                t_cache[0][j]->ack_tx[DIRECTION_SOUTH](ack[i][j + 1].north);
                t_cache[0][j]->buffer_full_status_tx[DIRECTION_SOUTH](buffer_full_status[i][j + 1].north);

                t_cache[0][j]->req_tx[DIRECTION_WEST](req[i][j].west);
                t_cache[0][j]->flit_tx[DIRECTION_WEST](flit[i][j].west);
                t_cache[0][j]->ack_tx[DIRECTION_WEST](ack[i][j].east);
                t_cache[0][j]->buffer_full_status_tx[DIRECTION_WEST](buffer_full_status[i][j].east);

                // Data NoC
                t_cache[0][j]->datareq_rx[DIRECTION_NORTH](datareq[i][j].south);
                t_cache[0][j]->dataflit_rx[DIRECTION_NORTH](dataflit[i][j].south);
                t_cache[0][j]->dataack_rx[DIRECTION_NORTH](dataack[i][j].north);
                t_cache[0][j]->databuffer_full_status_rx[DIRECTION_NORTH](databuffer_full_status[i][j].north);

                t_cache[0][j]->datareq_rx[DIRECTION_EAST](datareq[i + 1][j].west);
                t_cache[0][j]->dataflit_rx[DIRECTION_EAST](dataflit[i + 1][j].west);
                t_cache[0][j]->dataack_rx[DIRECTION_EAST](dataack[i + 1][j].east);
                t_cache[0][j]->databuffer_full_status_rx[DIRECTION_EAST](databuffer_full_status[i + 1][j].east);

                t_cache[0][j]->datareq_rx[DIRECTION_SOUTH](datareq[i][j + 1].north);
                t_cache[0][j]->dataflit_rx[DIRECTION_SOUTH](dataflit[i][j + 1].north);
                t_cache[0][j]->dataack_rx[DIRECTION_SOUTH](dataack[i][j + 1].south);
                t_cache[0][j]->databuffer_full_status_rx[DIRECTION_SOUTH](databuffer_full_status[i][j + 1].south);

                t_cache[0][j]->datareq_rx[DIRECTION_WEST](datareq[i][j].east);
                t_cache[0][j]->dataflit_rx[DIRECTION_WEST](dataflit[i][j].east);
                t_cache[0][j]->dataack_rx[DIRECTION_WEST](dataack[i][j].west);
                t_cache[0][j]->databuffer_full_status_rx[DIRECTION_WEST](databuffer_full_status[i][j].west);

                t_cache[0][j]->datareq_tx[DIRECTION_NORTH](datareq[i][j].north);
                t_cache[0][j]->dataflit_tx[DIRECTION_NORTH](dataflit[i][j].north);
                t_cache[0][j]->dataack_tx[DIRECTION_NORTH](dataack[i][j].south);
                t_cache[0][j]->databuffer_full_status_tx[DIRECTION_NORTH](databuffer_full_status[i][j].south);

                t_cache[0][j]->datareq_tx[DIRECTION_EAST](datareq[i + 1][j].east);
                t_cache[0][j]->dataflit_tx[DIRECTION_EAST](dataflit[i + 1][j].east);
                t_cache[0][j]->dataack_tx[DIRECTION_EAST](dataack[i + 1][j].west);
                t_cache[0][j]->databuffer_full_status_tx[DIRECTION_EAST](databuffer_full_status[i + 1][j].west);

                t_cache[0][j]->datareq_tx[DIRECTION_SOUTH](datareq[i][j + 1].south);
                t_cache[0][j]->dataflit_tx[DIRECTION_SOUTH](dataflit[i][j + 1].south);
                t_cache[0][j]->dataack_tx[DIRECTION_SOUTH](dataack[i][j + 1].north);
                t_cache[0][j]->databuffer_full_status_tx[DIRECTION_SOUTH](databuffer_full_status[i][j + 1].north);

                t_cache[0][j]->datareq_tx[DIRECTION_WEST](datareq[i][j].west);
                t_cache[0][j]->dataflit_tx[DIRECTION_WEST](dataflit[i][j].west);
                t_cache[0][j]->dataack_tx[DIRECTION_WEST](dataack[i][j].east);
                t_cache[0][j]->databuffer_full_status_tx[DIRECTION_WEST](databuffer_full_status[i][j].east);

                // TODO: check if hub signal is always required
                // signals/port when tile receives(rx) from hub
                t_cache[0][j]->hub_req_rx(req[i][j].from_hub);
                t_cache[0][j]->hub_flit_rx(flit[i][j].from_hub);
                t_cache[0][j]->hub_ack_rx(ack[i][j].to_hub);
                t_cache[0][j]->hub_buffer_full_status_rx(buffer_full_status[i][j].to_hub);

                // signals/port when tile transmits(tx) to hub
                t_cache[0][j]->hub_req_tx(req[i][j].to_hub); // 7, sc_out
                t_cache[0][j]->hub_flit_tx(flit[i][j].to_hub);
                t_cache[0][j]->hub_ack_tx(ack[i][j].from_hub);
                t_cache[0][j]->hub_buffer_full_status_tx(buffer_full_status[i][j].from_hub);
                ...
                if (it != GlobalParams::hub_for_tile.end())
                {
                    ...
                    hub[hub_id]->tile2port_mapping[t_cache[0][j]->local_id] = port;
                ...
                t_cache[0][j]->free_slots[DIRECTION_NORTH](free_slots[i][j].north);
                t_cache[0][j]->free_slots[DIRECTION_EAST](free_slots[i + 1][j].east);
                t_cache[0][j]->free_slots[DIRECTION_SOUTH](free_slots[i][j + 1].south);
                t_cache[0][j]->free_slots[DIRECTION_WEST](free_slots[i][j].west);

                t_cache[0][j]->free_slots_neighbor[DIRECTION_NORTH](free_slots[i][j].south);
                t_cache[0][j]->free_slots_neighbor[DIRECTION_EAST](free_slots[i + 1][j].west);
                t_cache[0][j]->free_slots_neighbor[DIRECTION_SOUTH](free_slots[i][j + 1].north);
                t_cache[0][j]->free_slots_neighbor[DIRECTION_WEST](free_slots[i][j].east);

                // NoP
                t_cache[0][j]->NoP_data_out[DIRECTION_NORTH](nop_data[i][j].north);
                t_cache[0][j]->NoP_data_out[DIRECTION_EAST](nop_data[i + 1][j].east);
                t_cache[0][j]->NoP_data_out[DIRECTION_SOUTH](nop_data[i][j + 1].south);
                t_cache[0][j]->NoP_data_out[DIRECTION_WEST](nop_data[i][j].west);

                t_cache[0][j]->NoP_data_in[DIRECTION_NORTH](nop_data[i][j].south);
                t_cache[0][j]->NoP_data_in[DIRECTION_EAST](nop_data[i + 1][j].west);
                t_cache[0][j]->NoP_data_in[DIRECTION_SOUTH](nop_data[i][j + 1].north);
                t_cache[0][j]->NoP_data_in[DIRECTION_WEST](nop_data[i][j].east);

                // Data NoC
                t_cache[0][j]->datafree_slots[DIRECTION_NORTH](datafree_slots[i][j].north);
                t_cache[0][j]->datafree_slots[DIRECTION_EAST](datafree_slots[i + 1][j].east);
                t_cache[0][j]->datafree_slots[DIRECTION_SOUTH](datafree_slots[i][j + 1].south);
                t_cache[0][j]->datafree_slots[DIRECTION_WEST](datafree_slots[i][j].west);

                t_cache[0][j]->datafree_slots_neighbor[DIRECTION_NORTH](datafree_slots[i][j].south);
                t_cache[0][j]->datafree_slots_neighbor[DIRECTION_EAST](datafree_slots[i + 1][j].west);
                t_cache[0][j]->datafree_slots_neighbor[DIRECTION_SOUTH](datafree_slots[i][j + 1].north);
                t_cache[0][j]->datafree_slots_neighbor[DIRECTION_WEST](datafree_slots[i][j].east);

                t_cache[0][j]->dataNoP_data_out[DIRECTION_NORTH](datanop_data[i][j].north);
                t_cache[0][j]->dataNoP_data_out[DIRECTION_EAST](datanop_data[i + 1][j].east);
                t_cache[0][j]->dataNoP_data_out[DIRECTION_SOUTH](datanop_data[i][j + 1].south);
                t_cache[0][j]->dataNoP_data_out[DIRECTION_WEST](datanop_data[i][j].west);

                t_cache[0][j]->dataNoP_data_in[DIRECTION_NORTH](datanop_data[i][j].south);
                t_cache[0][j]->dataNoP_data_in[DIRECTION_EAST](datanop_data[i + 1][j].west);
                t_cache[0][j]->dataNoP_data_in[DIRECTION_SOUTH](datanop_data[i][j + 1].north);
                t_cache[0][j]->dataNoP_data_in[DIRECTION_WEST](datanop_data[i][j].east);
            }
            else{
                char tile_name[64];
                Coord tile_coord;
                ...
                string(GlobalParams::routing_algorithm),
                "default");
                t[i][j]->dr->configure(tile_id,
                GlobalParams::stats_warm_up_time,
                GlobalParams::buffer_depth,
                grtable);
                t[i][j]->dr->power.configureRouter(GlobalParams::flit_size,
                GlobalParams::buffer_depth,
                GlobalParams::flit_size,
                string(GlobalParams::routing_algorithm),
                "default");
                ...
                // Data NoC
                t[i][j]->datareq_rx[DIRECTION_NORTH](datareq[i][j].south);
                t[i][j]->dataflit_rx[DIRECTION_NORTH](dataflit[i][j].south);
                t[i][j]->dataack_rx[DIRECTION_NORTH](dataack[i][j].north);
                t[i][j]->databuffer_full_status_rx[DIRECTION_NORTH](databuffer_full_status[i][j].north);

                t[i][j]->datareq_rx[DIRECTION_EAST](datareq[i + 1][j].west);
                t[i][j]->dataflit_rx[DIRECTION_EAST](dataflit[i + 1][j].west);
                t[i][j]->dataack_rx[DIRECTION_EAST](dataack[i + 1][j].east);
                t[i][j]->databuffer_full_status_rx[DIRECTION_EAST](databuffer_full_status[i + 1][j].east);

                t[i][j]->datareq_rx[DIRECTION_SOUTH](datareq[i][j + 1].north);
                t[i][j]->dataflit_rx[DIRECTION_SOUTH](dataflit[i][j + 1].north);
                t[i][j]->dataack_rx[DIRECTION_SOUTH](dataack[i][j + 1].south);
                t[i][j]->databuffer_full_status_rx[DIRECTION_SOUTH](databuffer_full_status[i][j + 1].south);

                t[i][j]->datareq_rx[DIRECTION_WEST](datareq[i][j].east);
                t[i][j]->dataflit_rx[DIRECTION_WEST](dataflit[i][j].east);
                t[i][j]->dataack_rx[DIRECTION_WEST](dataack[i][j].west);
                t[i][j]->databuffer_full_status_rx[DIRECTION_WEST](databuffer_full_status[i][j].west);

                t[i][j]->datareq_tx[DIRECTION_NORTH](datareq[i][j].north);
                t[i][j]->dataflit_tx[DIRECTION_NORTH](dataflit[i][j].north);
                t[i][j]->dataack_tx[DIRECTION_NORTH](dataack[i][j].south);
                t[i][j]->databuffer_full_status_tx[DIRECTION_NORTH](databuffer_full_status[i][j].south);

                t[i][j]->datareq_tx[DIRECTION_EAST](datareq[i + 1][j].east);
                t[i][j]->dataflit_tx[DIRECTION_EAST](dataflit[i + 1][j].east);
                t[i][j]->dataack_tx[DIRECTION_EAST](dataack[i + 1][j].west);
                t[i][j]->databuffer_full_status_tx[DIRECTION_EAST](databuffer_full_status[i + 1][j].west);

                t[i][j]->datareq_tx[DIRECTION_SOUTH](datareq[i][j + 1].south);
                t[i][j]->dataflit_tx[DIRECTION_SOUTH](dataflit[i][j + 1].south);
                t[i][j]->dataack_tx[DIRECTION_SOUTH](dataack[i][j + 1].north);
                t[i][j]->databuffer_full_status_tx[DIRECTION_SOUTH](databuffer_full_status[i][j + 1].north);

                t[i][j]->datareq_tx[DIRECTION_WEST](datareq[i][j].west);
                t[i][j]->dataflit_tx[DIRECTION_WEST](dataflit[i][j].west);
                t[i][j]->dataack_tx[DIRECTION_WEST](dataack[i][j].east);
                t[i][j]->databuffer_full_status_tx[DIRECTION_WEST](databuffer_full_status[i][j].east);
                ...
                // Data NoC
                t[i][j]->datafree_slots[DIRECTION_NORTH](datafree_slots[i][j].north);
                t[i][j]->datafree_slots[DIRECTION_EAST](datafree_slots[i + 1][j].east);
                t[i][j]->datafree_slots[DIRECTION_SOUTH](datafree_slots[i][j + 1].south);
                t[i][j]->datafree_slots[DIRECTION_WEST](datafree_slots[i][j].west);

                t[i][j]->datafree_slots_neighbor[DIRECTION_NORTH](datafree_slots[i][j].south);
                t[i][j]->datafree_slots_neighbor[DIRECTION_EAST](datafree_slots[i + 1][j].west);
                t[i][j]->datafree_slots_neighbor[DIRECTION_SOUTH](datafree_slots[i][j + 1].north);
                t[i][j]->datafree_slots_neighbor[DIRECTION_WEST](datafree_slots[i][j].east);

                t[i][j]->dataNoP_data_out[DIRECTION_NORTH](datanop_data[i][j].north);
                t[i][j]->dataNoP_data_out[DIRECTION_EAST](datanop_data[i + 1][j].east);
                t[i][j]->dataNoP_data_out[DIRECTION_SOUTH](datanop_data[i][j + 1].south);
                t[i][j]->dataNoP_data_out[DIRECTION_WEST](datanop_data[i][j].west);

                t[i][j]->dataNoP_data_in[DIRECTION_NORTH](datanop_data[i][j].south);
                t[i][j]->dataNoP_data_in[DIRECTION_EAST](datanop_data[i + 1][j].west);
                t[i][j]->dataNoP_data_in[DIRECTION_SOUTH](datanop_data[i][j + 1].north);
                t[i][j]->dataNoP_data_in[DIRECTION_WEST](datanop_data[i][j].east);
            }
		}
	}
	...
	for (int i = 0; i <= GlobalParams::mesh_dim_x; i++)
	{
		...
		datareq[i][0].south = 0;
		dataack[i][0].north = 0;
		datareq[i][GlobalParams::mesh_dim_y].north = 0;
		dataack[i][GlobalParams::mesh_dim_y].south = 0;

		datafree_slots[i][0].south.write(NOT_VALID);
		datafree_slots[i][GlobalParams::mesh_dim_y].north.write(NOT_VALID);

		datanop_data[i][0].south.write(tmp_NoP);
		datanop_data[i][GlobalParams::mesh_dim_y].north.write(tmp_NoP);
	}
	...
	for (int j = 0; j <= GlobalParams::mesh_dim_y; j++)
	{
		...
		datareq[0][j].east = 0;
		dataack[0][j].west = 0;
		datareq[GlobalParams::mesh_dim_x][j].west = 0;
		dataack[GlobalParams::mesh_dim_x][j].east = 0;

		datafree_slots[0][j].east.write(NOT_VALID);
		datafree_slots[GlobalParams::mesh_dim_x][j].west.write(NOT_VALID);

		datanop_data[0][j].east.write(tmp_NoP);
		datanop_data[GlobalParams::mesh_dim_x][j].west.write(tmp_NoP);
	}
}
