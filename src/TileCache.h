 ...
 *
 * This file contains the declaration of the TileCache
 */

#ifndef __NOXIMTileCacheCACHE_H__
#define __NOXIMTileCacheCACHE_H__
...
SC_MODULE(TileCache)
{
    SC_HAS_PROCESS(TileCache);

    // I/O Ports
    sc_in_clk clock;		                // The input clock for the TileCache
    sc_in <bool> reset;	                        // The reset signal for the TileCache
    ...
    // Signals required for Router-c_nic connection
    ...
    CacheNIC *c_nic;	                // Processing Element instance
	DataRouter *dr;

    // Constructor

    TileCache(sc_module_name nm, int id): sc_module(nm) 
	{
    	local_id = id;
		...
		c_nic = new CacheNIC("CacheNIC");
		c_nic->clock(clock);
		c_nic->reset(reset);

		c_nic->flit_rx(flit_rx_local);
		c_nic->req_rx(req_rx_local);
		c_nic->ack_rx(ack_rx_local);
		c_nic->buffer_full_status_rx(buffer_full_status_rx_local);
		
		c_nic->flit_tx(flit_tx_local);
		c_nic->req_tx(req_tx_local);
		c_nic->ack_tx(ack_tx_local);
		c_nic->buffer_full_status_tx(buffer_full_status_tx_local);


		c_nic->dataflit_rx(dataflit_rx_local);
		c_nic->datareq_rx(datareq_rx_local);
		c_nic->dataack_rx(dataack_rx_local);
		c_nic->databuffer_full_status_rx(databuffer_full_status_rx_local);
		
		c_nic->dataflit_tx(dataflit_tx_local);
		c_nic->datareq_tx(datareq_tx_local);
		c_nic->dataack_tx(dataack_tx_local);
		c_nic->databuffer_full_status_tx(databuffer_full_status_tx_local);
		...
		c_nic->free_slots_neighbor(free_slots_neighbor_local);
		c_nic->datafree_slots_neighbor(datafree_slots_neighbor_local);
    }
};
